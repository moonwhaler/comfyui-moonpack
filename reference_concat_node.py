...
class ReferenceConcat:
    ...
    SEARCH_ALIASES = ["concat", "reference", "identity", "first frame", "kjnodes", "video"]
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "main_image": ("IMAGE", {
                    "tooltip": "Main / first-frame image. If a batch is passed, only the first frame is used.",
                }),
                ...
                "ref_scale": ("FLOAT", {
                    ...
                    "tooltip": (
                        "Nominal strip thickness as a fraction of the main image's cross-side, before "
                        "the strip is uniformly rescaled to exactly fill the main image's long side."
                    ),
                }),
                "offset": ("INT", {
                    ...
                    "tooltip": (
                        ...
                        "Clamped so the main image is never cropped or shrunk."
                    ),
                }),
                "fill": (["black", "white", "gray", "edge_average"], {
                    "default": "black",
                    "tooltip": "Background color for any gap created by a negative offset. 'edge_average' samples the main image's edge nearest the strip.",
                }),
                "invert_mask": ("BOOLEAN", {
                    "default": False,
                    "tooltip": "Flip the output mask so the main-image area is 1.0 instead of the strip area.",
                }),
            },
        }
    ...
    def concat(self, main_image, reference_images, side, ref_scale, offset, fill, invert_mask):
        main = main_image[:1]
